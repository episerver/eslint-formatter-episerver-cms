...
const rules = require("./tmp/resources").rules;
...
*/
    ...
    const output =
        // Header
        endOfLine +
        chalk.white.bgGreen(` ${headers[0].padEnd(ruleMaxLen)}${headers[1].padEnd(moduleMaxLen)}${headers[2].padEnd(usagesMaxLen)} ${endOfLine}`) +
        // Data
        tableData.map(row => chalk` {gray ${row.rule.padEnd(ruleMaxLen)}}${row.module.padEnd(moduleMaxLen)}${noteUsages(row.usages).padEnd(usagesMaxLen)} `).join(endOfLine) + endOfLine +
        // Summary
        chalk.keyword("orange")(`  ${deprecatedEmoji} Deprecated API's used ${totalDeprecated} times${endOfLine}`) +
        chalk.keyword("orange")(`  ${internalEmoji} Internal API's used ${totalInternal} times${endOfLine}`) +
        endOfLine;
